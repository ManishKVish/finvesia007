...
def get_last_position():
    ...
    if previous_symbol is not None and previous_quantity is not 0:
        if previous_quantity < default_quantiy and previous_quantity is not 0:
...
def get_exchange_Symbol_strikePrice_orderAreadyPlaced(symbol, message):
    ...
    if fst_target_order_id is not None:
...
def exit_order(symbol, message):
    ...
    if previous_symbol != None:
        ...
        if fst_target_order_id is not None:
            api.cancel_order(fst_target_order_id)
        if snd_target_order_id is not None:
...
def target_sl():
    global fst_target_order_id, sl_order_id, snd_target_order_id
    try:
        if fst_target is not None:
...
def get_status_of_order(orderNo):
    ...
    for index, order in rejected_orders.iterrows():
        ...
        if order["status"] == "COMPLETE":
            ...
            snd_target = float(target_percent) * float(order["avgprc"])
            snd_target = format(fst_target, ".1f")
    ...
    )  # Return None if the order is not found in the order book
