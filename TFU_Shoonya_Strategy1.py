...
import requests
from TradingSymbolPopulator import *


stock="NIFTY" # BANKNIFTY OR NIFTY

# First value should be either BANKNIFTY OR NIFTY
# Second Value is Expiry Date: Don't give month in CAPS (e.g. MAR). Give like Jan, Feb, Dec etc
# Third value should be the CURRENT INDEX (example 40600 for BANKNIFTY or 17700 for NIFTY)
my_information = instrumentListForStrategyFile(stock,'09-Mar-23',17500)

# instrumentList = instrumentListForSubscribeFile('NIFTY','09-Mar-23',17500) # Praveen: See comments above before running
...
df["PnL"] = [PnL]
df.to_csv('Str1_Shoonya.csv',mode='a',index=True,header=True)
